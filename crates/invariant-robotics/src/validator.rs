...
mod tests {
    ...
    fn happy_path_approved() {
        ...
        // 13 prior checks + SR2 (payload sensor-range) = 14.
        assert_eq!(result.signed_verdict.verdict.checks.len(), 14);
    ...
    fn multi_hop_chain_approved() {
        ...
        // Link via v11 1.2 mandatory predecessor binding.
        let mut claims = vec![
            Pca {
                p_0: "root".into(),
                ops: ops(&["actuate:*"]),
        ...
        invariant_core::authority::chain::link_chain_digests(&mut claims);
        let s0 = sign_pca(&claims[0], &sk1).unwrap();
        let s1 = sign_pca(&claims[1], &sk2).unwrap();
