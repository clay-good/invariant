...
/// ```
/// use std::collections::HashMap;
/// use invariant_robotics::models::command::{Command, JointState, CommandAuthority};
...
///
/// ```
/// use invariant_robotics::models::command::{LocomotionState, FootState};
///
...
///
/// ```
/// use invariant_robotics::models::command::FootState;
///
...
///
/// ```
/// use invariant_robotics::models::command::JointState;
///
...
///
/// ```
/// use invariant_robotics::models::command::CommandAuthority;
/// use invariant_robotics::models::authority::Operation;
///
...
impl invariant_core::ValidationInput for Command {
    ...
    fn content_hash(&self) -> [u8; 32] {
        ...
        // Canonical JSON via serde_json — stable for fixed struct order.
        let bytes = serde_json::to_vec(self).expect("Command serializes");
        Sha256::digest(&bytes).into()
    }

    fn summary(&self) -> String {
        format!(
            "robotics command seq={} source={} joints={}",
            self.sequence,
            self.source,
            self.joint_states.len()
        )
    }
}
