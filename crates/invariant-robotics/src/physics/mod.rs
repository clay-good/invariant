...
///
/// ```
/// use invariant_robotics::physics::run_all_checks;
...
pub fn run_all_checks(
    ...
    // SR2: payload-side sensor range plausibility. Always on (joint_states
    // are always present); rejects physically impossible encoder / velocity /
    // EE / force readings before any downstream P-check trusts them.
    results.push(environment::check_sensor_range_payload(command));

    // P21–P25 + SR1: Environmental awareness checks — only when command
    // carries environment_state.
...
pub fn run_environment_checks(command: &Command, profile: &RobotProfile) -> Vec<CheckResult> {
    ...
    // SR1: env-side sensor range plausibility — reject physically impossible
    // values before P21-P25 threshold checks. Always active, no config needed.
    results.push(environment::check_sensor_range_env(env));
