...
use chrono::Utc;
use invariant_robotics::authority::crypto::{generate_keypair, sign_pca};
use invariant_robotics::models::authority::{Operation, Pca, SignedPca};
use invariant_robotics::models::command::EndEffectorForce;
use invariant_robotics::models::verdict::SignedVerdict;
use invariant_robotics::validator::ValidatorConfig;
use rand::rngs::{OsRng, StdRng};
use rand::SeedableRng;
use thiserror::Error;

use crate::robotics::campaign::CampaignConfig;
use crate::robotics::injector::{inject, InjectionType};
use crate::robotics::reporter::{CampaignReport, CampaignReporter};
use crate::robotics::scenario::{ScenarioGenerator, ScenarioType};
...
///
/// ```
/// use invariant_sim::robotics::isaac::dry_run::DryRunError;
///
...
pub enum DryRunError {
    ...
    ProfileLoad(#[from] invariant_robotics::profiles::ProfileError),

    /// The validator could not be constructed from the loaded profile.
    #[error("validator construction failed: {0}")]
    ValidatorBuild(#[from] invariant_robotics::validator::ValidatorError),
    ...
    PcaSign(#[from] invariant_robotics::models::error::AuthorityError),
...
/// ```
/// use invariant_sim::robotics::campaign::{CampaignConfig, ScenarioConfig, SuccessCriteria};
/// use invariant_sim::robotics::isaac::dry_run::{run_dry_campaign, DryRunError};
///
/// let config = CampaignConfig {
...
fn load_profile(
    profile_spec: &str,
) -> Result<invariant_robotics::models::profile::RobotProfile, DryRunError> {
    // Try built-in names first.
    match invariant_robotics::profiles::load_builtin(profile_spec) {
        Ok(p) => return Ok(p),
        Err(invariant_robotics::profiles::ProfileError::UnknownProfile(_)) => {} // fall through to file
        ...
        .any(|c| c == std::path::Component::ParentDir)
    {
        return Err(DryRunError::ProfileLoad(
            invariant_robotics::profiles::ProfileError::UnknownProfile(
    ...
    let bytes = std::fs::read(path).map_err(|_| {
        DryRunError::ProfileLoad(invariant_robotics::profiles::ProfileError::UnknownProfile(
            profile_spec.to_string(),
        ))
    })?;
    invariant_robotics::profiles::load_from_bytes(&bytes).map_err(DryRunError::ProfileLoad)
}
...
fn select_scenario<'a>(
    ...
    total_weight: f64,
    prefix: &[f64],
    scenarios: &'a [crate::robotics::campaign::ScenarioConfig],
) -> &'a crate::robotics::campaign::ScenarioConfig {
...
fn parse_scenario_type(name: &str) -> Result<ScenarioType, DryRunError> {
    match name {
        ...
        // H: Temporal & sequence
        "SequenceReplay" | "sequence_replay" => Ok(ScenarioType::SequenceReplay),
        "SequenceGap" | "sequence_gap" => Ok(ScenarioType::SequenceGap),
        "DeltaTimeAttack" | "delta_time_attack" => Ok(ScenarioType::DeltaTimeAttack),
        "StaleCommand" | "stale_command" => Ok(ScenarioType::StaleCommand),
        // C: Workspace & geometry
        "CorruptSpatialData" | "corrupt_spatial_data" => Ok(ScenarioType::CorruptSpatialData),
        // E: Force & manipulation
        "PayloadOverload" | "payload_overload" => Ok(ScenarioType::PayloadOverload),
        "ForceLimitSweep" | "force_limit_sweep" => Ok(ScenarioType::ForceLimitSweep),
        "GraspForceEnvelope" | "grasp_force_envelope" => Ok(ScenarioType::GraspForceEnvelope),
        "ForceRateSpike" | "force_rate_spike" => Ok(ScenarioType::ForceRateSpike),
        // H: Temporal & sequence (more)
        "FutureDatedSensor" | "future_dated_sensor" => Ok(ScenarioType::FutureDatedSensor),
        // F: Environment & sensors (single-phase splits)
        "TemperatureRamp" | "temperature_ramp" => Ok(ScenarioType::TemperatureRamp),
        "BatteryDrain" | "battery_drain" => Ok(ScenarioType::BatteryDrain),
        "LatencySpike" | "latency_spike" => Ok(ScenarioType::LatencySpike),
        "EStopEngageRelease" | "e_stop_engage_release" => Ok(ScenarioType::EStopEngageRelease),
        "SensorRangeImplausible" | "sensor_range_implausible" => {
            Ok(ScenarioType::SensorRangeImplausible)
        }
        "SensorPayloadRange" | "sensor_payload_range" => Ok(ScenarioType::SensorPayloadRange),
        "SensorFusionInconsistency" | "sensor_fusion_inconsistency" => {
            Ok(ScenarioType::SensorFusionInconsistency)
        }
        // D: Stability & locomotion (more)
        "ComStabilitySweep" | "com_stability_sweep" => Ok(ScenarioType::ComStabilitySweep),
        "WalkingGaitValidation" | "walking_gait_validation" => {
            Ok(ScenarioType::WalkingGaitValidation)
        }
        "StepOverextension" | "step_overextension" => Ok(ScenarioType::StepOverextension),
        "HeadingSpinout" | "heading_spinout" => Ok(ScenarioType::HeadingSpinout),
        "InclineWalking" | "incline_walking" => Ok(ScenarioType::InclineWalking),
        // C: Workspace & geometry (more)
        "WorkspaceBoundarySweep" | "workspace_boundary_sweep" => {
            Ok(ScenarioType::WorkspaceBoundarySweep)
        }
        "SelfCollisionApproach" | "self_collision_approach" => {
            Ok(ScenarioType::SelfCollisionApproach)
        }
        "OverlappingZoneBoundaries" | "overlapping_zone_boundaries" => {
            Ok(ScenarioType::OverlappingZoneBoundaries)
        }
        // K: Recovery & resilience (more)
        "EstopRecoveryCycle" | "estop_recovery_cycle" => Ok(ScenarioType::EstopRecoveryCycle),
        // L: Long-running stability (more)
        "MillionEntryAudit" | "million_entry_audit" => Ok(ScenarioType::MillionEntryAudit),
        "CounterSaturation" | "counter_saturation" => Ok(ScenarioType::CounterSaturation),
        // M: Cross-platform stress
        "ValidInvalidAlternating" | "valid_invalid_alternating" => {
            Ok(ScenarioType::ValidInvalidAlternating)
        }
        "MaximumPayloadCommand" | "maximum_payload_command" => {
            Ok(ScenarioType::MaximumPayloadCommand)
        }
        "MinimumValidCommand" | "minimum_valid_command" => Ok(ScenarioType::MinimumValidCommand),
        // J: Multi-step compound attacks (more)
        "NanAuthorityBypass" | "nan_authority_bypass" => Ok(ScenarioType::NanAuthorityBypass),
        "ProfileProbingTargeted" | "profile_probing_targeted" => {
            Ok(ScenarioType::ProfileProbingTargeted)
        }
        "MultiRobotDistraction" | "multi_robot_distraction" => {
            Ok(ScenarioType::MultiRobotDistraction)
        }
        // K: Recovery & resilience (more)
        "WatchdogRecoveryCycle" | "watchdog_recovery_cycle" => {
            Ok(ScenarioType::WatchdogRecoveryCycle)
        }
        // I: Cognitive escapes
        "DistractionFlooding" | "distraction_flooding" => Ok(ScenarioType::DistractionFlooding),
        "ErrorMining" | "error_mining" => Ok(ScenarioType::ErrorMining),
        "GradualDriftEscape" | "gradual_drift_escape" => Ok(ScenarioType::GradualDriftEscape),
        "SemanticConfusion" | "semantic_confusion" => Ok(ScenarioType::SemanticConfusion),
        "WatchdogTimeoutReplay" | "watchdog_timeout_replay" => {
            Ok(ScenarioType::WatchdogTimeoutReplay)
        }
        "TimingExploitation" | "timing_exploitation" => Ok(ScenarioType::TimingExploitation),
        "RateStressSustained" | "rate_stress_sustained" => Ok(ScenarioType::RateStressSustained),
        "Iso15066HumanProximityForce" | "iso15066_human_proximity_force" => {
            Ok(ScenarioType::Iso15066HumanProximityForce)
        }
        "BimanualCoordination" | "bimanual_coordination" => Ok(ScenarioType::BimanualCoordination),
        "MixedProfilesAudit" | "mixed_profiles_audit" => Ok(ScenarioType::MixedProfilesAudit),
        "ProfileProbingBinarySearch" | "profile_probing_binary_search" => {
            Ok(ScenarioType::ProfileProbingBinarySearch)
        }
        "RollbackReplay" | "rollback_replay" => Ok(ScenarioType::RollbackReplay),
        "ProfileReloadDuringOperation" | "profile_reload_during_operation" => {
            Ok(ScenarioType::ProfileReloadDuringOperation)
        }
        ...
        "ForgedSignature" | "forged_signature" => Ok(ScenarioType::ForgedSignature),
        "PrivilegeEscalation" | "privilege_escalation" => Ok(ScenarioType::PrivilegeEscalation),
        "ExpiredChain" | "expired_chain" => Ok(ScenarioType::ExpiredChain),
        "KeySubstitution" | "key_substitution" => Ok(ScenarioType::KeySubstitution),
        "ProvenanceMutation" | "provenance_mutation" => Ok(ScenarioType::ProvenanceMutation),
        "WildcardExploit" | "wildcard_exploit" => Ok(ScenarioType::WildcardExploit),
        ...
        "CrossChainSplice" | "cross_chain_splice" => Ok(ScenarioType::CrossChainSplice),
...
fn is_expected_reject(scenario: ScenarioType) -> bool {
    !matches!(
        ...
        ScenarioType::Baseline
            ...
            // H-03: sequence gaps are explicitly *allowed* by spec
            // (multi-source model, spec-v7 §2.7). Physics + authority are
            // baseline-safe, so every command should be APPROVED.
            | ScenarioType::SequenceGap
            ...
            // Category F single-phase splits (F-01..F-04) are MIXED:
            // the ramp scenarios start under the warning band (PASS) and
            // cross the hard limit later (REJECT); F-04 alternates
            // engage/release. Classify as not-expected-reject so the
            // dry-run harness doesn't count early valid commands as
            // violation escapes.
            | ScenarioType::TemperatureRamp
            | ScenarioType::BatteryDrain
            | ScenarioType::LatencySpike
            | ScenarioType::EStopEngageRelease
            // F-07 sensor-fusion inconsistency: `check_sensor_fusion` is a
            // standalone helper that is not yet wired into the per-command
            // validator. Generator emits diverging Position readings every
            // command, but the dry-run validator does not currently flag
            // them — classify as not-expected-reject so we don't count
            // unflagged commands as violation escapes. Re-classify once
            // the fusion check joins the validator's check set.
            | ScenarioType::SensorFusionInconsistency
            // Category D extras: D-01 cycles four COM positions, three of
            // which pass and one rejects; D-02 is the legitimate gait
            // happy-path (every command PASSES); D-07/D-08/D-10 are
            // monotonic ramps that cross their respective limits mid-way.
            // None of these are pure-reject — classify as
            // not-expected-reject so the dry-run harness doesn't count
            // the in-band commands as violation escapes.
            | ScenarioType::ComStabilitySweep
            | ScenarioType::WalkingGaitValidation
            | ScenarioType::StepOverextension
            | ScenarioType::HeadingSpinout
            | ScenarioType::InclineWalking
            // C-01 boundary sweep alternates inside-corner (PASS) with
            // outside-corner (REJECT). C-04 self-collision approach is a
            // ramp from valid → invalid. Both are mixed.
            | ScenarioType::WorkspaceBoundarySweep
            | ScenarioType::SelfCollisionApproach
            // K-03 is engage block then release block — mixed reject/pass.
            // L-02 / L-03 are baseline-safe long sequences (every command PASS).
            | ScenarioType::EstopRecoveryCycle
            | ScenarioType::MillionEntryAudit
            | ScenarioType::CounterSaturation
            // M-02 alternates pass/reject; M-05 is pure-PASS minimal
            // command. M-04 (max payload) trips name-mismatch — leave
            // in default expected-reject bucket.
            | ScenarioType::ValidInvalidAlternating
            | ScenarioType::MinimumValidCommand
            // J-06 is probe phase (PASS) then targeted attack (REJECT) — mixed.
            // J-03 and J-08 are pure-reject patterns; leave in default group.
            | ScenarioType::ProfileProbingTargeted
            // K-02 is normal → missed-heartbeat → normal (3-phase mixed).
            // I-02 is 9 valid + 1 invalid per cycle (mixed mostly-pass).
            // I-05 is pure-reject (every command trips a check) — leave default.
            | ScenarioType::WatchdogRecoveryCycle
            | ScenarioType::DistractionFlooding
            // I-01: gradual drift escape — most commands sit within
            // bounds (PASS) until the cumulative `0.0001 rad / step`
            // drift crosses `max` (REJECT P1). Mixed pattern; do not
            // count early in-band commands as violation escapes.
            | ScenarioType::GradualDriftEscape
            // I-09: timing exploitation alternates pass / reject at
            // 1 kHz (mixed); M-01 rate-stress is baseline-safe at
            // 1 kHz (pure-PASS). Both belong outside the default
            // expected-reject bucket.
            | ScenarioType::TimingExploitation
            | ScenarioType::RateStressSustained
            // M-06: mixed profiles in single audit log — baseline-safe
            // physics on every command, only the source-id rotation
            // varies. Pure-PASS.
            | ScenarioType::MixedProfilesAudit
            // I-07: binary-search probe stays inside [mid, max] on every
            // command → pure-PASS. K-05: profile-reload scenario is
            // baseline-safe; the harness handles the actual reload.
            | ScenarioType::ProfileProbingBinarySearch
            | ScenarioType::ProfileReloadDuringOperation
            // I-06 watchdog manipulation: phases A+B miss heartbeats /
            // drop authority (REJECT) but phase C re-establishes both
            // (PASS at generator level). Mixed pattern; don't count
            // phase-C commands as violation escapes.
            // K-06 validator restart: baseline-safe pre/post restart;
            // per-source sequence resets cleanly on source change.
            // Pure-PASS at the generator level.
            | ScenarioType::WatchdogManipulation
            | ScenarioType::ValidatorRestart
            // G-01 valid authority chain: pass-through chain +
            // baseline-safe physics → pure-PASS at the generator
            // level (the harness supplies a real chain).
            | ScenarioType::ValidAuthorityChain
            // Category N red-team fuzz (v11 2.11): N-01 and N-10
            // are mixed pass/reject patterns; N-02 and N-08 stay in
            // the default expected-reject bucket because every command
            // either lands an out-of-bounds mutation or carries an
            // adversarial unicode joint name.
...
fn make_error_verdict(
    ...
    use invariant_robotics::models::verdict::{AuthoritySummary, CheckResult, Verdict};
    SignedVerdict {
        verdict: Verdict {
            approved: false,
...
mod tests {
    use super::*;
    use crate::robotics::campaign::{CampaignConfig, ScenarioConfig, SuccessCriteria};
    ...
    fn dry_run_works_with_all_builtin_profiles() {
        for profile_name in invariant_robotics::profiles::list_builtins() {
            let config = CampaignConfig {
