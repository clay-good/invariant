...
//   - Per-connection liveness state: each connection lives in its own
//     `tokio::spawn` task whose `read_timeout` (from `BridgeConfig`) is
//     applied per-message in isolation. A silent or slow client closes
//     only its own task; other connections continue uninterrupted.
//     Regression test: `bridge_watchdog_per_connection_isolation`.
//   - The bridge-wide `BridgeStats` is the only shared mutable state
//     (an Arc<Mutex<>> of aggregate counters); it does not gate liveness.
...
use invariant_robotics::models::command::{Command, JointState};
...
// ---------------------------------------------------------------------------
// Fuzzing entry points (v12-N-12)
// ---------------------------------------------------------------------------

/// Maximum bytes per line the bridge will buffer before rejecting a frame.
/// Matches the bounded-read invariant landed in commits `70aefe6` / `54a2508`.
/// Anything past this limit is dropped without further parsing.
pub const FUZZ_BRIDGE_MAX_LINE_BYTES: usize = 8 * 1024;

/// Classification of a parsed bridge frame. Public so the fuzz harness
/// can assert structural invariants without re-parsing.
#[derive(Debug)]
pub enum FuzzBridgeFrameKind {
    /// A heartbeat with `{"heartbeat": true}`.
    Heartbeat,
    /// A heartbeat with `{"heartbeat": false}` — rejected at the message
    /// layer but parses cleanly.
    HeartbeatFalse,
    /// A `Command` JSON the validator would dispatch.
    Command,
    /// Bytes are too long to be a valid frame.
    Oversize,
    /// JSON did not match either incoming-message variant.
    Invalid,
}

/// Synchronous, panic-free counterpart of [`handle_message`]'s parse path.
///
/// Takes a single newline-framed line as bytes, enforces the bounded-read
/// invariant, and returns the frame kind. **Does not** run the validator —
/// the goal of the fuzz target is to exercise the JSON layer and the
/// untagged-enum dispatch, not the (heavy, async, profile-dependent)
/// physics pipeline. Returns `Err` only when the bytes are not UTF-8.
///
/// Used by [`fuzz/fuzz_targets/bridge_handle_line.rs`](../../../../../fuzz/fuzz_targets/bridge_handle_line.rs).
pub fn fuzz_bridge_handle_line(bytes: &[u8]) -> Result<FuzzBridgeFrameKind, std::str::Utf8Error> {
    if bytes.len() > FUZZ_BRIDGE_MAX_LINE_BYTES {
        return Ok(FuzzBridgeFrameKind::Oversize);
    }
    let raw = std::str::from_utf8(bytes)?;
    match serde_json::from_str::<IncomingMessage>(raw) {
        Ok(IncomingMessage::Heartbeat { heartbeat: true }) => Ok(FuzzBridgeFrameKind::Heartbeat),
        Ok(IncomingMessage::Heartbeat { heartbeat: false }) => {
            Ok(FuzzBridgeFrameKind::HeartbeatFalse)
        }
        Ok(IncomingMessage::Command(_)) => Ok(FuzzBridgeFrameKind::Command),
        Err(_) => Ok(FuzzBridgeFrameKind::Invalid),
    }
}

/// Convenience wrapper: split arbitrary bytes on `\n` and feed each line to
/// [`fuzz_bridge_handle_line`]. Returns the per-line kinds so the harness can
/// also assert "every input produces one result per line".
...
mod tests {
    ...
    use invariant_robotics::models::authority::{Operation, Pca};
    ...
    // ---- v12-N-12: fuzz handler entry points ----

    #[test]
    fn fuzz_handle_line_classifies_heartbeats() {
        let kind = fuzz_bridge_handle_line(br#"{"heartbeat":true}"#).unwrap();
        assert!(matches!(kind, FuzzBridgeFrameKind::Heartbeat));
        let kind = fuzz_bridge_handle_line(br#"{"heartbeat":false}"#).unwrap();
        assert!(matches!(kind, FuzzBridgeFrameKind::HeartbeatFalse));
    }

    #[test]
    fn fuzz_handle_line_oversize_short_circuits() {
        let big = vec![b'a'; FUZZ_BRIDGE_MAX_LINE_BYTES + 1];
        let kind = fuzz_bridge_handle_line(&big).unwrap();
        assert!(matches!(kind, FuzzBridgeFrameKind::Oversize));
    }

    #[test]
    fn fuzz_handle_line_invalid_classifies_as_invalid() {
        let kind = fuzz_bridge_handle_line(b"{not json").unwrap();
        assert!(matches!(kind, FuzzBridgeFrameKind::Invalid));
        let kind = fuzz_bridge_handle_line(b"").unwrap();
        assert!(matches!(kind, FuzzBridgeFrameKind::Invalid));
    }

    #[test]
    fn fuzz_handle_line_non_utf8_errors() {
        // 0xC3 0x28 is an invalid UTF-8 sequence.
        let result = fuzz_bridge_handle_line(&[0xC3, 0x28]);
        assert!(result.is_err());
    }

    #[test]
    fn fuzz_handle_multiline_yields_one_result_per_split() {
        let input = b"{\"heartbeat\":true}\n\n{\"heartbeat\":false}\n";
        let results = fuzz_bridge_handle_multiline(input);
        // `split('\n')` on `b"...\n...\n...\n"` yields 4 items (the last
        // trailing empty); we assert the handler keeps that 1:1 mapping.
        assert_eq!(results.len(), input.split(|&b| b == b'\n').count());
        // Spot-check: lines 0 and 2 must be Heartbeat / HeartbeatFalse.
        assert!(matches!(results[0], FuzzBridgeFrameKind::Heartbeat));
        assert!(matches!(results[2], FuzzBridgeFrameKind::HeartbeatFalse));
    }
    ...
    /// v11 3.2: per-connection watchdog isolation.
    ///
    /// One slow / silent client must not stall any other connection's
    /// liveness. Connection A is opened and immediately goes silent —
    /// the bridge's per-message `read_timeout` should fire on A's spawned
    /// task in isolation, while connection B continues to receive
    /// heartbeat acks straight through the supposed window of starvation.
    #[tokio::test]
    async fn bridge_watchdog_per_connection_isolation() {
        let (validator, _chain_b64) = make_validator();
        let socket_path = unique_socket_path();

        // Short read timeout so the test stays under a second total.
        let mut config = BridgeConfig::new(socket_path.clone(), validator, 0);
        config.read_timeout = std::time::Duration::from_millis(200);
        config.write_timeout = std::time::Duration::from_millis(500);
        ...
        // Connection A: open and never send. The bridge's read loop will
        // hit `read_timeout`, emit an error response, and disconnect.
        let stream_a = UnixStream::connect(&socket_path).await.unwrap();
        let (reader_a, _writer_a) = stream_a.into_split();
        let mut buf_a = BufReader::new(reader_a);

        // Connection B: open and heartbeat throughout A's silence window.
        let stream_b = UnixStream::connect(&socket_path).await.unwrap();
        let (reader_b, mut writer_b) = stream_b.into_split();
        let mut buf_b = BufReader::new(reader_b);

        // B heartbeats continuously, well under its own read_timeout (200 ms),
        // for a window that overflows A's read_timeout. If the watchdog
        // state were shared (or A's timeout poisoned the listener), B's
        // ack stream would stop. We require at least 8 successful acks
        // spanning ≥400 ms — twice A's read_timeout.
        let start = std::time::Instant::now();
        let mut acks_after_a_timeout = 0;
        for i in 0..8 {
            let mut line = String::new();
            writer_b
                .write_all(b"{\"heartbeat\": true}\n")
                .await
                .expect("B writer should remain alive across A's timeout");
            buf_b
                .read_line(&mut line)
                .await
                .expect("B reader should keep receiving acks across A's timeout");
            let resp: BridgeResponse = serde_json::from_str(line.trim()).unwrap();
            assert_eq!(
                resp.response_type, "heartbeat_ack",
                "ack {i} should arrive even after A's per-connection watchdog fires"
            );
            if start.elapsed() >= std::time::Duration::from_millis(220) {
                acks_after_a_timeout += 1;
            }
            tokio::time::sleep(std::time::Duration::from_millis(60)).await;
        }
        assert!(
            acks_after_a_timeout >= 3,
            "expected B to keep acking past A's read_timeout window, \
             got {acks_after_a_timeout} acks after the 220 ms mark"
        );

        // A's task must have torn itself down; the read end either yields
        // the timeout-error response and then EOF, or just EOF if the
        // error response raced past us. We don't depend on the exact
        // message, only that the read side is no longer pending.
        let drain = tokio::time::timeout(
            std::time::Duration::from_millis(200),
            buf_a.read_line(&mut String::new()),
        )
        .await;
        assert!(
            drain.is_ok(),
            "A's read side should have completed (Ok(0) or err) once its watchdog fired"
        );
