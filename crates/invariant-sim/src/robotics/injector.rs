...
use invariant_robotics::models::command::{Command, EndEffectorPosition};
...
///
/// ```
/// use invariant_sim::robotics::injector::InjectionType;
///
...
/// use invariant_sim::robotics::injector::list_injections;
...
fn inject_proximity_overspeed(cmd: &mut Command, profile: &RobotProfile) {
    ...
    if cmd.end_effector_positions.is_empty() {
        cmd.end_effector_positions
            .push(invariant_robotics::models::command::EndEffectorPosition {
...
mod tests {
    ...
    fn self_collision_creates_collision_pair_named_ees() {
        ...
        let profile = panda();
        let authority = invariant_robotics::models::command::CommandAuthority {
            pca_chain: "dGVzdA==".to_owned(),
            required_ops: vec![],
        };
        let ops = vec![];
