...
use invariant_robotics::models::actuation::SignedActuationCommand;
...
mod tests {
    ...
    fn forge_mode_valid_command_returns_0() {
        ...
        use invariant_robotics::models::command::EndEffectorPosition;
        let cmd = Command {
            timestamp: chrono::Utc::now(),
            source: "test".to_string(),
            sequence: 1,
    ...
    // -----------------------------------------------------------------------
    // v12-N-17: --fail-on-audit-error regression
    //
    // `invariant robotics validate` exits non-zero whenever an audit-log write
    // fails *after* the log has been successfully opened. The original v12
    // prompt names a `--fail-on-audit-error` flag, but on `validate` this is
    // unconditional behaviour: every audit-write failure aborts the run with
    // exit 2 and a stderr message that mentions the audit log. (The flag
    // exists separately on `serve`, where it gates whether HTTP 503 is
    // returned per-request — covered by
    // `test_audit_write_failure_returns_503_when_fail_on_audit_error`.)
    //
    // To force a post-open write failure we point `--audit-log` at
    // `/dev/full`, which can be opened in append mode but returns ENOSPC on
    // every write. The special device is Linux-only; on other platforms the
    // test compiles as a no-op so cross-platform CI stays green.

    #[cfg(target_os = "linux")]
    #[test]
    fn validate_exits_nonzero_when_audit_write_fails() {
        let profile_tmp = write_profile();
        let (key_tmp, _sk) = write_key_file();
        ...
        if !dev_full.exists() {
            // Some sandboxed runners omit /dev/full; treat as a soft skip.
            eprintln!("validate_exits_nonzero_when_audit_write_fails: /dev/full absent — skipping");
            return;
        }
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn validate_exits_zero_when_audit_write_succeeds() {
        // Mirror of the negative path above with the same Forge-mode command:
        // a writable audit log under a tempdir succeeds end-to-end. Locks
        // down the contract that the failure case is exclusive to /dev/full.
        let dir = TempDir::new().unwrap();
        let profile_tmp = write_profile();
        let (key_tmp, _sk) = write_key_file();
        ...
        assert_eq!(
            exit, 0,
            "validate must exit zero when the audit log is writable"
        );
        assert!(
            audit_log.exists() && std::fs::metadata(&audit_log).map(|m| m.len()).unwrap_or(0) > 0,
            "happy-path audit log must have at least one record"
        );
    }

    #[cfg(not(target_os = "linux"))]
    #[test]
    fn validate_audit_write_failure_test_is_linux_only() {
        // Documented limitation: the negative path uses /dev/full which is a
        // Linux-only special device. The contract is exercised on Linux CI
        // (v12-N-17). This stub keeps grep results consistent across OSes.
        eprintln!("v12-N-17 validate /dev/full test is Linux-only; skipping on this OS");
    }
