> Superseded by [docs/robotics/spec.md](../../robotics/spec.md) as of 2026-05-19. Kept for historical reference (the file itself was already marked SUPERSEDED in v11 5.16).

% Invariant — Gap-Closure Specification (Verified)
% Status: SUPERSEDED — every gap below has been mapped to a v7-v12 prompt;
%         see the reconciliation table in §0a. Retained for historical
%         reference; v12 N-9 will move this file to `docs/history/`.
% Date: 2026-04-27 (original); reconciled 2026-05-16 under v11-5.16
% Branch: codelicious/spec-spec-15m-campaign-part-2
% Supersedes: prior `docs/spec-gaps.md` draft (2026-04-27)

# 0a. Reconciliation against v7-v12 prompts (v11-5.16, 2026-05-16)

Status legend: **CLOSED** (work landed, with citation), **PARTIAL** (work
started, see citation for the open piece), **DUP** (subsumed by a later
prompt — execute that prompt to close), **NEW** (still open with no
later prompt covering it). After this reconciliation no gap is orphan.

| §   | Title                                            | Status  | Pointer |
|-----|--------------------------------------------------|---------|---------|
| 1.1 | B1-B4 execution-binding invariants               | DUP     | `docs/robotics/spec-v11.md` Prompt 1.1 (re-verified open in v12 §1). |
| 1.2 | A3 PCA-to-PCA causal binding                     | DUP     | `spec-v11.md` Prompt 1.2 |
| 1.3 | G-07 wildcard + G-09 splice tests                | DUP     | acceptance fixtures under `spec-v11.md` Prompts 1.1 / 1.2; reporting hook in v12 N-1 (DONE). |
| 2.1 | Hardware key stores (OS-keyring / TPM / YubiHSM) | PARTIAL | Backends still stubs (`spec-v11.md` Prompt 4.1, OPEN). Fail-fast typed errors added: v12 N-13 (DONE, `crates/invariant-cli/src/robotics/commands/keygen.rs`). |
| 2.2 | Audit replication & external witness             | DUP     | `spec-v11.md` Prompt 4.2 |
| 2.3 | Alert sinks                                      | DUP     | `spec-v11.md` Prompt 4.3 |
| 3.1 | Proof package Merkle tree + signed manifest      | DUP     | `spec-v11.md` Prompts 1.3 + 1.4; format-version envelope queued in v12 N-5. |
| 3.2 | `campaign assemble` CLI                          | DUP     | `spec-v11.md` Prompt 1.5; `--resume` flag specified in v12 N-6. |
| 3.3 | Scenario coverage (~22/104)                      | PARTIAL | Reporting hook landed: v12 N-1 + N-2 (DONE). Missing generators still under `spec-v11.md` Prompts 2.1-2.11. |
| 3.4 | Isaac Lab envs cover one cell                    | DUP     | `spec-v11.md` Prompt 3.1 |
| 3.5 | Roadmap Steps 5-7 (cost ceiling, shadow runbook) | CLOSED  | Cost ceiling + SIGTERM checkpoints: v12 N-7 (`scripts/run_15m_campaign.sh`, `scripts/check_spend.py`). Shadow-deployment runbook: v12 N-8 (`docs/shadow-deployment.md`). |
| 4.1 | SR1 / SR2 share one check name                   | DUP     | `spec-v11.md` Prompt 5.1 |
| 4.2 | Profiles missing `end_effectors`                 | CLOSED  | Commit `274f8dc` (cited in `spec-v12.md` §1, row 5.2). |
| 4.3 | Multi-robot coordinator proven pairwise only     | PARTIAL | Partition-merge soundness at the boundary: v12 N-18 (DONE, `crates/invariant-coordinator/tests/partition_merge_soundness.rs`). N-way generalisation deferred. |
| 4.4 | Watchdog shared across bridge connections        | PARTIAL | Bounded reads landed; per-connection watchdog isolation still OPEN under `spec-v11.md` Prompt 3.2. |
| 4.5 | Documentation drift on counts                    | CLOSED  | Scenario count surfaced runtime via `ScenarioType::all()` + spec-ID mapping: v12 N-1 / N-2. |
| 5.1 | Lean formalization is sketch + axioms            | DUP     | `spec-v11.md` Prompt 5.9 |
| 5.2 | SBOM + reproducible-build verification           | PARTIAL | SBOM in CI: CLOSED (release.yml cyclonedx step, cited in `spec-v12.md` row "SBOM in CI"). Reproducible-build attestation: NEW, queued as v13 candidate (per `spec-v12.md` §5 "Out of scope"). |
| 5.3 | ROS 2 bindings unreferenced                      | CLOSED  | Disposition runbook: `spec-v11.md` Prompt 5.11 (DONE this batch, `docs/ros2.md` and README cross-link). |
| 5.4 | Spec consolidation                               | DUP     | `spec-v12.md` Prompt N-9 (must run last). |

**Net effect:** of the 19 gaps, 4 are CLOSED outright (3.5, 4.2, 4.5, 5.3),
5 are PARTIAL with the closed slice cited, 9 are DUP pointing to a v11/v12
prompt that owns the remaining work, and 1 (the reproducible-build half of
5.2) is NEW and parked as a v13 candidate. Zero orphans.

The body of §1-§9 below is preserved unchanged for historical reference;
do not edit it. Future updates should go in the prompt files
(`spec-v11.md`, `spec-v12.md`) and be reflected back in the table above.
