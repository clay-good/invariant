...
**Physical Invariants (25 checks + sensor range pre-filter):**

The core 10 checks (P1-P10) are listed here. For the full 25-check set including manipulation (P11-P14), locomotion (P15-P20), and environmental (P21-P25), see Section 25.
...
| P16 | Foot clearance bounds (min + max_step_height) | Locomotion |
...
### Data Quality Pre-Filters (2 checks)

| # | Check | Domain |
|---|-------|--------|
| SR1 | Sensor range plausibility (EnvironmentState) | Reject physically impossible IMU, temperature, battery, latency values |
| SR2 | Sensor payload range plausibility (SensorPayload) | Reject physically impossible position, force, encoder, COM, GRF values |

These run before the numbered invariants as defense-in-depth guards. They are not numbered invariants but produce `CheckResult` entries in the verdict.

**Total: 34 numbered invariants + 2 data quality pre-filters.** Each is deterministic. Each produces a signed pass/fail. Each is independently testable.
...
### Phase 48: P16 Maximum Step Height Enforcement (Step 97) — ✅ COMPLETE

97. ~~**P16 max_step_height upper-bound enforcement**: Closed the gap between spec-v1.md Section 2.2 P16 specification and implementation. The spec defines P16 as checking *both* `foot_height >= min_foot_clearance` (lower bound) and `foot_height <= max_step_height` (upper bound), but only the lower bound was implemented. Added `max_step_height: f64` field to `LocomotionConfig` with `#[serde(default)]` at 0.5 m for backward compatibility. Updated P16 `check_foot_clearance()` to reject swing feet that exceed `max_step_height` — catches stomping (foot raised excessively high slams down with dangerous force). Added profile validation: `max_step_height` must be finite, positive, and greater than `min_foot_clearance`. Updated all 3 locomotion profiles with appropriate values (Spot: 0.20 m, Unitree G1: 0.30 m, Unitree H1: 0.35 m). Updated all 8 test helpers constructing `LocomotionConfig`. Added 7 new P16 tests covering upper-bound pass/fail, contact foot exemption, mixed violations, both-bounds-violated, and infinity rejection. `cargo clippy -- -D warnings` clean, all 1,738 tests pass.~~ ✓

### Phase 49: P16 Stomp Adversarial Testing (Step 98) — ✅ COMPLETE

98. ~~**P16 stomp adversarial testing**: Added adversarial coverage for the `max_step_height` upper-bound check introduced in Step 97. New `StompViolation` injection type in `injector.rs` — sets swing foot z-position to 3× `max_step_height` from the profile's `LocomotionConfig`. New `LocomotionStomp` scenario type in `scenario.rs` — graduated foot height ramp from 0.5× to 3× `max_step_height`, where early commands pass and later commands must be rejected. Wired both into the dry-run campaign (`parse_injection_type`, `parse_scenario_type`). Updated injection count from 26 to 27, scenario count from 13 to 14 across all assertion lists. Added `StompViolation` to the locomotion exemption list in `adversarial_test.rs` (profiles without locomotion config skip the check). 3 injection tests (panda, quadruped, humanoid) verify swing foot exceeds `max_step_height`. 2 scenario generation tests (quadruped, humanoid) verify command count. 1 stomp-specific clearance test verifies the ramp produces feet above `max_step_height`. Cross-profile injection coverage updated to 27 types. `cargo clippy -- -D warnings` clean, all 1,744 tests pass.~~ ✓

### Phase 50: Edge-Case Hardening + StabilityConfig Validation (Step 99) — ✅ COMPLETE

99. ~~**Edge-case hardening and StabilityConfig validation**: Comprehensive boundary-condition test coverage across 6 subsystems — audit log corruption resilience (truncated line, blank lines, single-bit-flip detection), authority chain single-hop edge cases (sufficient/insufficient ops, empty required ops, expired PCA, max hops), P7 self-collision edge cases (identical positions with nonzero/zero threshold, NaN position), P10 proximity boundary edge cases (EE exactly on sphere boundary, just outside, NaN position), P8 delta_time edge cases (negative, zero, NaN, exactly-at-max), and zeroed signing key security documentation. Additionally discovered and fixed a validation gap: `StabilityConfig` had NO validation in `RobotProfile::validate()` — NaN polygon vertices would silently corrupt the ray-casting algorithm (edges touching NaN vertices are invisible, causing incorrect inside/outside classification). Added `StabilityConfigInvalid` error variant and validation rules: `com_height_estimate` must be finite and positive, enabled configs must have >= 3 polygon vertices, and all polygon vertex coordinates must be finite. 7 new stability validation tests (valid config, NaN com_height, zero com_height, enabled with < 3 vertices, disabled with < 3 vertices accepted, NaN vertex, infinity vertex). 19 pre-existing edge-case tests + 7 new validation tests = 26 tests total for Step 99. `cargo clippy -- -D warnings` clean, all 1,751 tests pass.~~ ✓

### Phase 51: Fail-Closed Hardening + P25 E-Stop Enforcement (Step 100) — ✅ COMPLETE

100. ~~**Fail-closed hardening and P25 e-stop enforcement**: Edge-case coverage across 5 subsystems — CycleCoordinator reset behavior (mid-cycle rejection, CycleComplete re-arms spindle zone, reset invariant), P6 exclusion zone NaN bounds fail-closed (AABB NaN bound, sphere NaN center, sphere NaN radius, sphere Inf radius — all correctly reject via the `point_in_aabb`/`point_in_sphere` NaN guards), P21-P25 environment checks silent skip (P21-P24 skipped when profile has no environment config, P25 always fires, serde defaults verification), sensor freshness validation (future-dated rejection, recent-past acceptance, stale reading expiry, batch future-dated rejection), sensor-physics binding gap documentation (RequireSigned with empty readings rejects). Additionally identified and tested a spec compliance gap: spec-v1.md Section 2.3 states "P25: This check CANNOT be disabled in any profile. It is always active." Added 2 new tests proving P25 enforcement: (1) e-stop engaged + no environment config = rejection (proves the "cannot be disabled" guarantee), (2) e-stop data absent (`None`) + no environment config = P25 still fires and passes (documents sensor-absence behavior). 14 pre-existing edge-case tests + 2 new P25 enforcement tests = 16 tests total for Step 100. `cargo clippy -- -D warnings` clean, all 1,753 tests pass.~~ ✓

### Phase 52: Overflow Safety + Subsystem Robustness (Step 101) — ✅ COMPLETE

101. ~~**u64 counter overflow safety fix + subsystem robustness hardening**: Edge-case coverage across 4 subsystems — digital twin NaN fail-closed (NaN position not classified as normal, Inf velocity rejected, NaN errors don't poison subsequent observations, window_size=0 rejected), proof package path traversal prevention (filenames with `/`, `\`, `..` rejected, safe names accepted, assemble rejects path traversal in report names), Merkle tree corruption detection (partial corruption produces different root, all-corrupt returns None, single entry is deterministic), and threat scorer u64 overflow safety. **Functional fix**: discovered that `authority_checks`, `authority_rejections[principal]`, and `DriftTracker::count` all used plain `+= 1` which would silently wrap at `u64::MAX` — wrapping `authority_checks` to 0 would re-enable the "not enough data" early return (threat scoring disabled) and cause division-by-zero in rejection rate calculation. Changed all three counters to `saturating_add(1)`. Added 2 new tests: (1) `authority_checks_saturate_at_u64_max` — sets counters to `u64::MAX - 1`, scores twice, asserts counters stay at `u64::MAX` and scores remain bounded; (2) `drift_tracker_count_saturates_at_u64_max` — sets count to `u64::MAX`, updates, asserts count stays at `u64::MAX`. 12 pre-existing edge-case tests + 2 new saturation tests = 14 tests total for Step 101. `cargo clippy -- -D warnings` clean, all 1,755 tests pass.~~ ✓

### Phase 53: Boundary-Condition Safety + Watchdog Clock Robustness (Step 102) — ✅ COMPLETE

102. ~~**Boundary-condition safety across authority, physics, and watchdog**: Edge-case coverage across 4 subsystems — operation wildcard matching edge cases (wildcard doesn't match bare prefix, short prefix covers deep path, different root rejected, bare wildcard covers single segment, case sensitivity), P18 friction cone zero-coefficient behavior (zero tangential on zero-friction passes, any tangential on zero-friction fails — documents that profile validation prevents mu<=0 in production), P13 force rate with invalid delta_time (zero delta_time with matching EEs fails, NaN delta_time fails), and watchdog timing edge cases (latch is one-way, stale reset doesn't regress baseline, backwards heartbeat doesn't regress, large timeout no overflow, sentinel hash format). Additionally identified and tested a gap in watchdog clock robustness: `check()` was untested with `now_ms < last_heartbeat_ms` (backwards clock) and `now_ms == last_heartbeat_ms` (zero elapsed). Both are handled correctly by `saturating_sub` but were undocumented. Added 2 new tests: (1) `check_with_backwards_clock_does_not_false_trigger` — heartbeat at t=200, check at t=100, must NOT trigger; (2) `check_at_exact_heartbeat_time_does_not_trigger` — heartbeat at t=100, check at t=100, elapsed=0, must NOT trigger. 14 pre-existing edge-case tests + 2 new watchdog clock tests = 16 tests total for Step 102. `cargo clippy -- -D warnings` clean, all 1,757 tests pass.~~ ✓

### Phase 54: Command Size Cap + DoS Protection (Step 103) — ✅ COMPLETE

103. ~~**Command size cap DoS protection and joint name mismatch**: The `check_command_size_caps` function guards 5 collections against oversized adversarial payloads — `joint_states` (cap 256), `end_effector_positions` (cap 256), `end_effector_forces` (cap 256), `signed_sensor_readings` (cap 256), and `locomotion_state.feet` (cap 64). Commands exceeding any cap are immediately rejected before physics processing, preventing memory exhaustion from adversarial commands with millions of entries. Pre-existing tests covered `joint_states` and `end_effector_positions` overflow, a normal-sized command passthrough, extra joints beyond profile rejected, and a `check_command_size_caps` unit test. Identified a gap: 3 of 5 guarded collections (`end_effector_forces`, `signed_sensor_readings`, `locomotion_state.feet`) had no integration test proving the cap fires through the full validator pipeline. Added 3 new tests: (1) `oversized_end_effector_forces_rejected` — 300 forces, cap 256; (2) `oversized_sensor_readings_rejected` — 300 readings, cap 256; (3) `oversized_locomotion_feet_rejected` — 100 feet, cap 64. All verify the rejection verdict contains the correct `command_size_cap` check with the specific collection name in details. 5 pre-existing tests + 3 new collection cap tests = 8 tests total for Step 103. `cargo clippy -- -D warnings` clean, all 1,760 tests pass.~~ ✓

### Phase 55: Coordinator Security + ISO 15066 Fail-Closed Fix (Step 104) — ✅ COMPLETE

104. ~~**Coordinator security hardening and ISO 15066 NaN fail-closed fix**: Edge-case coverage across 2 subsystems — coordinator config validation (zero/negative/NaN separation rejected, positive accepted), coordinator runtime NaN/Inf position detection (NaN and Inf EE positions fail separation check), identical position detection (distance=0 violates 0.5m separation), and ISO 15066 `point_in_sphere` NaN handling. **Functional fix**: discovered that `point_in_sphere` guarded against NaN in `center` and `radius` (fail-closed) but NOT in `point` (the EE position). If `point[0]` was NaN, `dx*dx = NaN`, and `NaN <= r*r` evaluated to false — the point was treated as OUTSIDE the zone, silently bypassing ISO 15066 force limits (fail-open). Extended the finite guard to also check `point.iter().all(|v| v.is_finite())` so NaN/Inf EE positions are treated as inside the zone (fail-closed, defense-in-depth — the P10 and P5 checks also catch NaN EEs, but each check should be self-contained). Added `config_validate_rejects_zero_max_robots` test for the coordinator's max_robots guard. Added `point_in_sphere_nan_point_returns_true` and `point_in_sphere_inf_point_returns_true` tests proving the NaN point fix works. 11 pre-existing edge-case tests + 3 new tests = 14 tests total for Step 104. `cargo clippy -- -D warnings` clean, all 1,763 tests pass.~~ ✓

### Phase 56: Proximity NaN Fail-Closed + Margin Composition (Step 105) — ✅ COMPLETE

105. ~~**P10 proximity NaN fail-closed hardening and margin composition verification**: Edge-case coverage across 2 areas — P10 proximity zone NaN center/radius fail-closed (NaN zone center triggers velocity scaling, NaN zone radius triggers velocity scaling), and real-world margin composition tests (P1 position valid without margins rejected with margins, P2/P10 velocity scaling composes proximity + envelope + margins correctly, `run_all_checks` integrated margin tightening). **Functional fix**: discovered that `proximity.rs::point_in_sphere` (the P10 velocity-scaling sphere check) had the same NaN-point fail-open bug fixed in the ISO 15066 `point_in_sphere` in Step 104 — it guarded against NaN in `center` and `radius` but not `point`. Although the P10 caller independently rejects NaN EE positions at lines 34-51 before reaching `point_in_sphere`, the function itself was inconsistent with its ISO 15066 counterpart and would be fail-open if called from any other path. Extended the finite guard to also check `point.iter().all(|v| v.is_finite())` for defense-in-depth consistency. Added 1 new test `p10_proximity_nan_ee_position_caught_by_sphere_guard` proving NaN EE positions are rejected through the P10 integration path. 5 pre-existing edge-case tests + 1 new defense-in-depth test = 6 tests total for Step 105. `cargo clippy -- -D warnings` clean, all 1,764 tests pass.~~ ✓

### Phase 57: Replay Protection + Threat Scorer Poison Recovery + COSE Hardening (Step 106) — ✅ COMPLETE

106. ~~**Sequence replay protection, threat scorer poison recovery, and COSE malformed input hardening**: Three functional additions across 3 files — (1) `serve.rs`: added `last_sequence: AtomicU64` to `AppState` for command sequence monotonicity enforcement. The `/validate` handler rejects any command whose sequence is not strictly greater than the last accepted, preventing replay of previously-approved signed actuation commands. Uses `SeqCst` ordering for correctness under concurrent access. (2) `validator.rs`: threat scorer mutex now uses `unwrap_or_else(|poisoned| poisoned.into_inner())` for poison recovery — if a previous panic poisoned the mutex, validation continues rather than permanently bricking the validator. Added `validate_with_forces()` API that passes previous end-effector forces through to P13 force-rate check. (3) `authority/tests.rs`: COSE malformed input hardening — empty bytes, garbage bytes, and single null byte all return typed `CoseError`/`AuthorityError`, not panics. Pre-existing tests: validator determinism (identical commands produce identical verdicts), threat scorer poison recovery (intentional poison then validate succeeds), 3 COSE malformed input tests. **Gap identified**: the serve handler's replay protection had zero integration tests. Added 3 new serve handler tests exercising the sequence monotonicity through the full axum request pipeline: (1) `replay_same_sequence_rejected` — sequence=1 twice, second returns 400 with "replay" in error; (2) `lower_sequence_after_higher_rejected` — sequence=5 then 3, second returns 400; (3) `incrementing_sequence_accepted` — sequence=1 then 2, both return 200. Also added `Deserialize` to `ErrorResponse` to enable test deserialization. 5 pre-existing tests + 3 new serve replay tests = 8 tests total for Step 106. `cargo clippy -- -D warnings` clean, all 1,767 tests pass.~~ ✓

### Phase 58: spec-v1.md Status Synchronization (Step 107) — ✅ COMPLETE

107. ~~**spec-v1.md status synchronization with codebase reality**: The spec-v1.md simulation campaign specification had multiple stale markers that contradicted the actual codebase state, undermining its role as source of truth. Fixed 5 sections: (1) Section 2.2 header: "P11-P20 -- TO IMPLEMENT" → "P11-P20 -- IMPLEMENTED" (all 10 checks implemented in Steps 43-52). (2) Section 3.3 "Sensor Integrity (Future)": marked sensor staleness detection (Step 65/100) and signed sensor data (Step 64) as done with step references; sensor range validation and fusion consistency remain deferred. (3) Section 8.1 status table: updated from 264 tests / 4 profiles / 10 checks to 1,767 tests / 10 profiles / 25 checks / 14 scenarios / 27 injection types, added 7 new component rows (digital twin, coordinator, threat scoring, proof package, sensor integrity). (4) Section 8.2 "Must Build" table: marked 9 of 11 items as complete with step references; only Isaac Lab task environments and video replay script remain pending (both require GPU/RunPod hardware). (5) Section 8.4 Phase 0 readiness checklist: checked all 12 items, updated counts (10 profiles, 14 scenarios, 27 injections, 1,767+ tests). `cargo clippy -- -D warnings` clean, all 1,767 tests pass.~~ ✓

### Phase 59: Sensor Range Validation (Step 108) — ✅ COMPLETE

108. ~~**Sensor range plausibility validation**: Implemented `check_sensor_range()` in `environment.rs` — rejects physically impossible EnvironmentState values before they reach P21-P25 threshold checks. This catches corrupted sensor drivers or spoofed data reporting values outside the laws of physics. Plausibility bounds: IMU angles > ±π rad (physically impossible for a rigid body), temperature below absolute zero (-273.15°C) or above 1000°C, battery percentage outside [0, 100], negative communication latency. NaN values are deliberately skipped by the range check (already handled by each P21-P25 check's own NaN guard). Wired into `run_environment_checks()` as the first check — runs before P25 (e-stop) and P21-P24, always active regardless of profile environment config. Updated the `env_checks_skip_p21_p24_when_config_absent` test to expect 2 results (sensor_range + emergency_stop) instead of 1. Updated spec-v1.md Section 3.3 to mark sensor range validation as done. 18 new tests covering: empty env passes, valid/invalid IMU angles (including boundary at ±π), NaN skipped, valid/invalid temperature (including boundary at absolute zero), valid/invalid battery (including boundaries at 0% and 100%), valid/invalid latency (including zero), multiple violations reported together. `cargo clippy -- -D warnings` clean, all 1,785 tests pass.~~ ✓

### Phase 60: Signed Sensor Payload Range Validation (Step 109) — ✅ COMPLETE

109. ~~**Signed sensor payload range validation**: Step 108 added range validation for `EnvironmentState` (the unsigned sensor data path). Step 109 completes the coverage by adding `check_payload_range()` in `sensor.rs` for `SensorPayload` (the signed sensor data path through `SignedSensorReading`). Rejects physically impossible values that no real sensor can produce: position beyond ±1000 m, force/GRF beyond ±100,000 N, joint encoder position beyond ±4π rad, encoder velocity beyond ±1000 rad/s, center-of-mass beyond ±100 m, and NaN/Inf in any component. Added `PayloadOutOfRange` error variant to `SensorError`. Wired into `verify_sensor_batch()` pipeline after signature verification and freshness check — the sequence is now signature → freshness → range → accept. 17 new tests covering all 5 payload variants: Position (valid, NaN, beyond-1000m, at-boundary), Force (valid, beyond-100k, Inf), JointEncoder (valid, position-beyond-4π, velocity-beyond-1000, NaN-position, NaN-velocity), CenterOfMass (valid, beyond-100m), GroundReaction (valid, beyond-100k, NaN). `cargo clippy -- -D warnings` clean, all 1,802 tests pass.~~ ✓

### Phase 61: Invariant Table Synchronization (Step 110) — ✅ COMPLETE

110. ~~**Invariant table synchronization across both spec documents**: The authoritative invariant tables — used in the proof package and compliance mappings — had multiple stale entries after Steps 97-109 added new checks and modified existing ones. Fixed 4 items across 2 spec files: (1) spec.md Section 1.8: updated header from "Physical Invariants (10 checks)" to "Physical Invariants (25 checks + sensor range pre-filter)" with forward reference to Section 25 for the complete set. (2) spec.md Section 25 P16: updated description from "Foot clearance minimum" to "Foot clearance bounds (min + max_step_height)" to reflect Step 97's upper-bound addition. (3) spec.md Section 25: added new "Data Quality Pre-Filters (2 checks)" table documenting SR1 (`check_sensor_range` for EnvironmentState, Step 108) and SR2 (`check_payload_range` for SensorPayload, Step 109), with note that these produce CheckResult entries in verdicts but are not numbered invariants. Updated total from "34 invariants" to "34 numbered invariants + 2 data quality pre-filters." (4) spec-v1.md Section 2.3: updated header from "P21-P25 -- NEW" to "P21-P25 -- IMPLEMENTED". (5) spec-v1.md Section 2.4 check summary: added SR1-SR2 data quality pre-filter rows, updated total from 34 to "34 numbered + 2 pre-filters = 36 total checks in verdict." `cargo clippy -- -D warnings` clean, all 1,802 tests pass.~~ ✓
